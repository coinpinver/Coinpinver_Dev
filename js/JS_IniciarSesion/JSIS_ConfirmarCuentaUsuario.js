...
function validaFormConfirmaCuenta() {
	if($("#codigoSeguridad").val().length >= 1) {
		Accion_JQuery("confirmaCuentaSuscriptor", "SetConfirmaCuentaUsuario", "result-confirma", "Validando tu información", "Estamos validando tu cuenta", 1);
	} else {
...
function setCorreoConfirma() {
	Accion_JQuery("confirmaCuentaSuscriptor", "ReenviarConfirmaCuentaUsuario", "result-confirma", "Reenviando correo electrónico", "Ingresa a tu correo electrónico para visualizar tu codigo se seguridad", 1);
}
